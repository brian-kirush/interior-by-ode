...
function generateInvoicePdf(invoice, stream) {
    ...
    // Header
    doc.font(FONT_BOLD).fontSize(20).text('INVOICE', 50, 50);
    ...
    doc.font(FONT_REGULAR).text(invoice.client_name, 50, 165);
    doc.text(invoice.client_address || 'N/A', 50, 180);
    doc.text(invoice.client_email, 50, 195);
    ...
    doc.font(FONT_REGULAR).text(`$${invoice.subtotal.toFixed(2)}`, 0, currentY, { align: 'right' });
    currentY += 20;

    doc.font(FONT_BOLD).text('Tax:', totalsX, currentY, { align: 'left' });
    doc.font(FONT_REGULAR).text(`$${invoice.tax_amount.toFixed(2)}`, 0, currentY, { align: 'right' });
    currentY += 20;

    if (invoice.discount_amount > 0) {
        doc.font(FONT_BOLD).text('Discount:', totalsX, currentY, { align: 'left' });
        doc.font(FONT_REGULAR).text(`-$${invoice.discount_amount.toFixed(2)}`, 0, currentY, { align: 'right' });
        currentY += 20;
    }
    ...
    doc.font(FONT_BOLD).fontSize(12).text('Total Due:', totalsX, currentY, { align: 'left' });
    doc.font(FONT_BOLD).text(`$${invoice.total.toFixed(2)}`, 0, currentY, { align: 'right' });
    currentY += 40;

    // Notes
    if (invoice.notes) {
        doc.font(FONT_BOLD).fontSize(10).text('Notes:', 50, currentY);
        doc.font(FONT_REGULAR).text(invoice.notes, 50, currentY + 15, {
            align: 'left',
            width: 500,
        });
    }
